 ...
 *
 * 04-Sep-2021: Crudely patched / upgraded to handle webP, useful if you use Wordpress 5.8+
 *
 * $Rev$
 */
...
class timthumb {
	...
	protected $filePrependSecurityBlock = "<?php die('Execution denied!'); //"; //Designed to have three or four letter mime type, space, question mark and greater than symbol appended. 7 bytes total for webp and 6 for all other handled types.
	...
	public function __construct(){
		...
		$raw_src = $this->param('src');

		// Urldecode only if necessary
		if(substr( $raw_src, 0, 14 ) === "https%3A%2F%2F" ||  substr( $raw_src, 0, 13 ) === "http%3A%2F%2F"){
			$this->src = urldecode($raw_src);
		}else{
			$this->src = $raw_src;
		}
		...
		if($this->isURL){
			...
			if(! $this->localImage){
				...
				asort($arr);
				$this->cachefile = $this->cacheDirectory . '/' . FILE_CACHE_PREFIX . $cachePrefix . md5($this->salt . implode('', $arr) . $this->fileCacheVersion) . FILE_CACHE_SUFFIX;
				$this->debug(1, "Could not find the local image so treating it as an external resource.");
				//$this->error("Could not find the internal image you specified.".' '.$this->param('src'));
				//$this->set404();
				//return false;
			}else{
	...
	protected function serveErrors(){
		...
		echo '<br />TimThumb version : JIG</pre>';
	}
	...
	protected function processImageAndWriteToCache($localImage){
		...
		$origType = $sData[2];
		$mimeType = $sData['mime'];
        // M: adapting this to manage webp
		$this->debug(3, "In processImageAndWriteToCache. Mime type of image is $mimeType");
		if(! preg_match('/^image\/(?:gif|jpg|jpeg|png|webp)$/i', $mimeType)){
			return $this->error("The image being resized is not a valid gif, jpg, webp or png.");
		}
		...
		} //MB: added to patch webp images into timthumb
		...
		file_put_contents($tempfile4, $this->filePrependSecurityBlock . $imgType . ' ?' . '>'); //6 or 7 extra bytes, first 3 or 4 being image type
	...
	// MB: Some updating and reworking is needed here as webP is a 4-letter imgtype
	protected function serveCacheFile(){
		$webPOffset = 0; // this is either 0 for non webp files, or 1 for webp files.
		...
		fseek($fp, strlen($this->filePrependSecurityBlock), SEEK_SET); // MB: reads after the securiy block
		$imgType = fread($fp, 4); // MB: this needed to be patched to 4 chars because of webp
		$this->debug(3, sprintf("serveCacheFile: ftell = %u, end of security prefix: %u", ftell($fp), strlen($this->filePrependSecurityBlock)));
		if(strtolower($imgType) === 'webp') {
			$webPOffset = 1;
		} else {
			$imgType = substr($imgType, 0, 3); // we restore it to be the first 3 chars
			fseek($fp, -1, SEEK_CUR); // and we set the file pointer correctly
		}
		fseek($fp, 3, SEEK_CUR);
		$this->debug(3, sprintf("serveCacheFile: image initial bytes mark it as %s. webPoffset = %u, ftell = %u, end of security block: %u", $imgType, $webPOffset, ftell($fp), strlen($this->filePrependSecurityBlock) + 6 + $webPOffset ));
		if(ftell($fp) != strlen($this->filePrependSecurityBlock) + 6 + $webPOffset ){
			@unlink($this->cachefile);
			return $this->error("The cached image file seems to be corrupt.");
		}
		$imageDataSize = filesize($this->cachefile) - (strlen($this->filePrependSecurityBlock) + 6 + $webPOffset);
		$this->debug(3, "about to call sendImageHeaders with imgType = ".$imgType);
		...
		if ($content != FALSE) {
			$content = substr($content, strlen($this->filePrependSecurityBlock) + 6 - $webPOffset);
			echo $content;
	...
	protected function sendImageHeaders($mimeType, $dataSize){
		...
		$this->debug(3, "In sendImageHeaders. Mime Type is $mimeType");
	...
	protected function serveImg($file){
		...
		$this->debug(3, "in serveImg. Content-Type = ".$s['mime']);
		...
		$bytes = @readfile($file);
		if($bytes > 0){
			$this->debug(3, "in serveImg. readfile branch, returning true");

			return true;
		}
		$content = @file_get_contents ($file);
		if ($content != FALSE){
			$this->debug(3, "in serveImg. file_get_contents branch, returning true");
			echo $content;
			return true;
		}
		return false;
